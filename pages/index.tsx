...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (memberParam && memberSelect) {
      ...
      const matchingOption = Array.from(memberSelect.options).find(
        (option) =>
          option.value === memberParam ||
          decodeURIComponent(option.value) === decodeURIComponent(memberParam as string),
      )
  ...
  }, [router.query.member, router.isReady])
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    // Validate form
    const donorNameElement = document.getElementById("donor-name") as HTMLInputElement
    const donorEmailElement = document.getElementById("donor-email") as HTMLInputElement
    const memberSelectElement = document.getElementById("member-select") as HTMLSelectElement
    const amountElement = document.getElementById("amount") as HTMLInputElement

    if (!donorNameElement || !donorEmailElement || !memberSelectElement || !amountElement) {
      showMessage("Form elements not found. Please refresh the page.")
      return
    }

    const donorName = donorNameElement.value
    const donorEmail = donorEmailElement.value
    const memberSelected = memberSelectElement.value
    const memberText = memberSelectElement.options[memberSelectElement.selectedIndex].text
    ...
    try {
      ...
      if (!stripe || !card) {
        showMessage("Payment processing not available. Please try again later.")
        setIsLoading(false)
        return
      }
  ...
  const createWonderlandElement = () => {
    ...
    element.style.opacity = (0.1 + Math.random() * 0.4).toString()
